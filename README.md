...
```bash
lambda-demo-4$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
lambda-demo-4$ sam local start-api
lambda-demo-4$ curl http://localhost:3000/
```
...
```bash
lambda-demo-4$ sam logs -n HelloWorldFunction --stack-name lambda-demo-4 --tail
```
...
```bash
sam delete --stack-name lambda-demo-4
```
